...
def parsear_csv_tabla_individual(ruta_csv):
    """
    Parsea un CSV de una sola tabla (formato T_*.csv) y devuelve una lista de campos.
    """
    campos = []
    cabecera = None
    with open(ruta_csv, "r", encoding="utf-8", newline='') as f:
        ...
        for row in reader:
            ...
            if not cabecera:
                continue
            if len(row) < len(cabecera):
                row += ["" for _ in range(len(cabecera) - len(row))]
            if len(row) > len(cabecera):
                row = row[:len(cabecera)-1] + [",".join(row[len(cabecera)-1:])]
            campo = dict(zip(cabecera, row))
            campos.append(campo)
    return campos
...
def main():
    parser = argparse.ArgumentParser(description="Parsea formatoSalida.csv o todos los T_*.csv de un directorio y exporta cada tabla principal como YAML.")
    parser.add_argument("--csv", help="Ruta al archivo formatoSalida.csv (modo master)")
    parser.add_argument("--csv-dir", help="Directorio con los CSVs por tabla (T_*.csv)")
    ...
    if args.csv_dir:
        # Modo por tabla: procesar todos los T_*.csv en el directorio
        input_dir = Path(args.csv_dir)
        csv_files = sorted([f for f in input_dir.iterdir() if f.is_file() and f.name.upper().startswith("T_") and f.suffix.lower() == ".csv"])
        tablas = {}
        metadatos = {}
        for csv_path in csv_files:
            nombre_tabla = csv_path.stem.replace("T_", "").replace(".CSV", "").replace(".csv", "").upper()
            campos = parsear_csv_tabla_individual(str(csv_path))
            if not campos:
                continue
            tablas[nombre_tabla] = campos
            # Metadatos básicos: campos obligatorios, tipos, etc.
            meta = {}
            if campos:
                meta["campos"] = [c.get("Campo") for c in campos if c.get("Campo")]
                meta["primary_key"] = [c.get("Campo") for c in campos if c.get("Obligatorio", "").lower() == "sí" or c.get("Obligatorio", "").lower() == "si"]
            metadatos[nombre_tabla] = meta
        exportar_tablas_a_yaml(tablas, args.out)
        exportar_metadatos_logicos(metadatos, args.meta_yaml)
    elif args.csv:
        # Modo master CSV (actual)
        tablas = parsear_csv_formato(args.csv)
        exportar_tablas_a_yaml(tablas, args.out)
        metadatos = extraer_metadatos_logicos(args.csv)
        exportar_metadatos_logicos(metadatos, args.meta_yaml)
    else:
        parser.error("Debe especificar --csv (master) o --csv-dir (por tabla)")
