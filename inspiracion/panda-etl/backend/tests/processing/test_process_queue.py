...
@patch('app.processing.process_queue.extract_data')
@patch('app.processing.process_queue.ChromaDB')
def test_extract_process_uses_vector_segments(mock_chroma, mock_extract_data):
    vectorstore = Mock()
    mock_chroma.return_value = vectorstore
    vectorstore.get_relevant_docs.return_value = {
        "metadatas": [[{"page_number": 1, "previous_sentence_id": -1, "next_sentence_id": -1}]],
        "documents": [["Segment"]]
    }
    vectorstore.get_relevant_docs_by_id.return_value = {
        "metadatas": [[{}]],
        "documents": [[""]]
    }
    ...
    def extract_side_effect(api_key, fields, file_path=None, pdf_content=None):
        captured["pdf_content"] = pdf_content
        return ExtractFieldsResponse(fields=[], references=[])

    mock_extract_data.side_effect = extract_side_effect

    process = Mock(id=1, project_id=1, details={"fields": [{"key": "field1"}]})
    process_step = Mock(id=1, asset=Mock(id=1))
    asset_content = Mock(content={"word_count": 1000, "content": [{"text": "Test"}]})

    extract_process("api", process, process_step, asset_content)

    assert captured["pdf_content"] == "Segment"
    vectorstore.get_relevant_docs.assert_called()


@patch('app.processing.process_queue.logger')
@patch('app.processing.process_queue.extract_data')
@patch('app.processing.process_queue.ChromaDB')
def test_extract_process_vectorstore_failure(mock_chroma, mock_extract_data, mock_logger):
    vectorstore = Mock()
    mock_chroma.return_value = vectorstore
    vectorstore.get_relevant_docs.side_effect = Exception("db error")
    ...
    process = Mock(id=1, project_id=1, details={"fields": [{"key": "field1"}]})
    process_step = Mock(id=1, asset=Mock(id=1))
    asset_content = Mock(content={"word_count": 1000, "content": [{"text": "Doc"}]})

    extract_process("api", process, process_step, asset_content)

    assert captured["pdf_content"] == "Doc"
    mock_logger.error.assert_called()
...
def test_update_process_step_status():
    ...
    mock_repo = MagicMock(spec=IProcessRepository)
    ...
    update_process_step_status(
        ...
        mock_status,
        mock_output,
        mock_output_references,
        repo=mock_repo,
    )

    mock_repo.update_process_step_status.assert_called_once_with(
        mock_db,
        mock_process_step,
        mock_status,
        output=mock_output,
